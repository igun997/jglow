...
class Display_Comment {
	...
	public function enqueue_scripts() {
		if ( is_product() && is_single() ) {
			wp_enqueue_style( "wvr-front-comment", WVR_PLUGIN_URL . "/assets/css/front-comment.css" );

			$css_option = Data::get_data_option();
			$custom_css = implode( ' ', $css_option['custom_css'] );
			$custom_css .= ".wvr-customer-pick .wvr-select-sample-cmt{color: {$css_option['canned_text_color']}; background-color:{$css_option['canned_bg_color']};}";
			$custom_css .= ".wvr-customer-pick .wvr-select-sample-cmt:hover{color: {$css_option['canned_text_hover_color']}; background-color:{$css_option['canned_hover_color']};}";
			$custom_css .= ".wvr-product-purchased{color: {$css_option['purchased_text_color']}; background-color:{$css_option['purchased_bg_color']};}";
			$custom_css .= ".wvr-icon-purchased{color: {$css_option['purchased_icon_color']};}";
			$custom_css .= ".wvr-icon-purchased:before{content:'\\" . $css_option['purchased_label_icon'] . "'; margin-right:5px}";
			wp_add_inline_style( 'wvr-front-comment', $custom_css );

			wp_enqueue_script( 'fast-comment', WVR_JS_URL . 'front-script.js', array( 'jquery' ), true, true );
			$datas = Data::get_data_option();
			$auto_rating   = isset( $datas['auto_rating'] ) ? $datas['auto_rating'] : '';
			$first_comment = isset( $datas['first_comment'] ) ? esc_html( $datas['first_comment'] ) : '';
	...
	public function sv_add_wc_review_notes( $review_form ) {
		...
		$datas                = Data::get_data_option();
		$sample_cmts          = isset( $datas['cmt_frontend'] ) ? $datas['cmt_frontend'] : '';
		$show_canned          = isset( $datas['show_canned'] ) ? $datas['show_canned'] : '';
		$canned_style_desktop = isset( $datas['canned_style_desktop'] ) ? $datas['canned_style_desktop'] : 'select';
		$canned_style_mobile  = isset( $datas['canned_style_mobile'] ) ? $datas['canned_style_mobile'] : 'select';
	...
	public function show_comments( $comment ) {
		if ( $comment->comment_type == 'comment' || ( $comment->comment_type == 'review' && ! $comment->user_id ) ) {
			return;
		}

		$datas = Data::get_data_option();
		if ( isset( $datas['show_purchased_label'] ) && $datas['show_purchased_label'] ) {
			global $product;
			...
			if ( $comment_author_id > 0 ) { //real cmt
				...
				if ( $product->is_type( 'variable' ) ) {
					foreach ( $result as $var_id => $qty ) {
						...
						$string .= ( "<span class='wvr-product-purchased'>" . ( wc_get_formatted_variation( $var->get_variation_attributes(), true ) ) . " x " . $qty . "</span>" );
					}
				} else {
				...
				if ( $product->is_type( 'variable' ) ) {
					...
					if ( isset( $var_id ) && is_numeric( $var_id ) ) {
						$var    = wc_get_product( $var_id );
						$string .= ( "<span class='wvr-product-purchased'>" . ( wc_get_formatted_variation( $var->get_variation_attributes(), true ) ) . " x 1</span>" );
					}
				} else {
			...
			if ( $string ) {
				echo '<div class="wvr-comments-group"><i class="wvr-icon-purchased wvr-purchased-format"></i>' . $string . "</div>";
			}

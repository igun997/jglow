	...
	*/
	...
	public function append_content_before_woocommerce_footer( $emailclass_object ) {		
		if ( isset( $emailclass_object->id ) && ( 'customer_new_account' === $emailclass_object->id ) ) {
			...
			$expire_time =  get_option('cev_verification_code_expiration', 600);
	...
	public function cev_resend_verification_email() {
		if ( isset( $_GET['cev_redirect_limit_resend'] ) && '' !== $_GET['cev_redirect_limit_resend'] ) { // WPCS: input var ok, CSRF ok.
			$user_id = base64_decode( $_GET['cev_redirect_limit_resend'] ); // WPCS: input var ok, CSRF ok.
	...
	public function cev_verify_user_email_with_pin_fun(){
		...
		$cev_verification_code_expiration = get_option('cev_verification_code_expiration', 600);
		
		if($cev_verification_code_expiration != 'never'){
		...
		$cev_pin = wc_clean($_POST['cev_pin1']);		
		...
		if($cev_email_verification_pin['pin'] == $cev_pin){
			...
			echo json_encode( array('success' => 'true','url' => get_permalink($redirect_page_id)) );die();			
		} else{
			echo json_encode( array('success' => 'false') );die();			
		}
		exit;

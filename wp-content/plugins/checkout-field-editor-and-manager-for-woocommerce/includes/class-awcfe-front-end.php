    ...
    public function email_after_order_table($order)
    {
        ...
        if ($awcf_data) {
            ...
            foreach ($awcf_data as $section => $fields) {
                ...
                if ($fields) {
                    $outString1 .= '<tr class="awcfe-' . $section . '-extra-items" ><th colspan="2" style="border: 1px solid #e5e5e5;" >' . __(ucfirst($sectionName), 'woocommerce') . ' ' . __('Extra Fields', 'checkout-field-editor-and-manager-for-woocommerce') . ' </td></tr>';
                }
